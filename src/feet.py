...
def screw_place():
    ...
    washer_r = 12.5 / 2
    screw_r = 3 / 2
    return (cylinder(r2=washer_r, r1=washer_r * 1.25, h=h, center=True) + cylinder(r=screw_r, h=30, center=True,
                                                                                   _fn=20)).up(h / 2 - 0.1)


def flap_raw(feet_width, edge_depth, inside_depth, flap_width, flap_height, feet_height):
    p = [(feet_width / 2 - edge_depth, -feet_width / 2), (feet_width / 2, -feet_width / 2),
         (feet_width / 2, -feet_width / 2 + flap_width + 3), (feet_width / 2 - 3, -feet_width / 2 + flap_width),
...
def flap_raw_wip(edge_depth, inside_depth, flap_width, flap_height, feet_height):
    p = [(- edge_depth, 0), (0, 0), (0, flap_width + 3), (- 3, flap_width), (- inside_depth, flap_width)]
    s = linear_extrude(flap_height + 0.05)(polygon(points=p))
    base = s.up(feet_height).down(flap_height)
    return base
...
def feet():
    ...
    breaking_point_top = cube([top_depth / 2+10, 0.1,0.1], center=True).right(bottom_width / 4).up(feet_height-1)

    breaking_point = hull()(breaking_point_top , breaking_point_bottom)
    return [(feet_wide-breaking_point, "feet")]
